...
import ass.management.admin.test.word.example.SegmentData;
import ass.management.admin.test.word.example.StoryData;
...
public class MyStoryExample {
	...
	public void testStoryExample() throws Exception {
		...
		s1.setPicture(new PictureRenderData(510, 402, "src/test/resources/word/story/image1.jpeg"));
		segments.add(s1);
		...
		s2.setPicture(new PictureRenderData(510, 510, "src/test/resources/word/story/image2.jpeg"));
		segments.add(s2);
		...
		s3.setPicture(new PictureRenderData(510, 384, "src/test/resources/word/story/image3.jpeg"));
		segments.add(s3);
		...
		s4.setPicture(new PictureRenderData(510, 510, "src/test/resources/word/story/image4.jpeg"));
		segments.add(s4);
		...
		s5.setPicture(new PictureRenderData(510, 412, "src/test/resources/word/story/image5.jpeg"));
		segments.add(s5);
		...
		DocxRenderData segment = new DocxRenderData(new File("src/test/resources/word/story/segment.docx"), segments );
		data.setSegment(segment);

		XWPFTemplate template = XWPFTemplate.compile("src/test/resources/word/story/story.docx").render(data);

		FileOutputStream out = new FileOutputStream("C:\\Users\\Administrator\\Desktop\\新建文件夹\\out_story.docx");
		template.write(out);
		out.flush();
		out.close();
		template.close();
