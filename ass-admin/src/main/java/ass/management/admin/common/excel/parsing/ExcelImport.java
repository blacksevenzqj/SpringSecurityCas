...
public class ExcelImport extends AbstractExcelResolver {
    ...
    public ExcelImportResult readExcel(String id, int titleIndex, InputStream excelStream, Integer sheetIndex,
                                       boolean multivalidate, Map<String, Object> map) throws Exception {
        //从注册信息中获取Bean信息
        ...
        return doReadExcel(excelDefinition, titleIndex, excelStream, sheetIndex, multivalidate, map);
    }

    protected ExcelImportResult doReadExcel(ExcelDefinition excelDefinition, int titleIndex, InputStream excelStream,
                                            Integer sheetIndex, boolean multivalidate, Map<String, Object> map) throws Exception {
    ...
    protected <T> List<T> readRows(List<ExcelError> errors, ExcelDefinition excelDefinition, List<String> titles,
                                   Sheet sheet, int titleIndex, boolean multivalidate, Map<String, Object> map) throws Exception {
        ...
        for (int i = titleIndex + 1; i <= rowNum; i++) {
            Row row = sheet.getRow(i);
            Object bean = readRow(errors, excelDefinition, row, titles, startRow + i, multivalidate, map);
            listBean.add((T) bean);
        }
        return listBean;
    }
     ...
     * @throws Exception
     */
    protected Object readRow(List<ExcelError> errors, ExcelDefinition excelDefinition, Row row, List<String> titles, int rowNum, boolean multivalidate, Map<String, Object> map) throws Exception {
		//创建注册时配置的bean类型
				...
				if(title.equals(titles.get(j))){
					try{
						...
						if(value != null){
							...
							value = super.convert(bean, map, fieldValue, Type.IMPORT, rowNum);
