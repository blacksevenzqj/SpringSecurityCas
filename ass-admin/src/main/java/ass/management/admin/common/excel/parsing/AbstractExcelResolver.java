...
import ass.management.admin.common.excel.util.ExcelUtil;
import ass.management.admin.common.utils.ReflectUtil;
import ass.management.common.utils.SpringContextUtils;
...
public abstract class AbstractExcelResolver implements CellValueConverter {
    ...
    //默认实现
    @Override
    public Object convert(Object bean, Map<String, Object> map, FieldValue fieldValue, Type type, int rowNum)
            throws Exception {
        if (map != null && !map.isEmpty() && map.get(ExcelConfig.Convert.DEFAULT_KEY) != null) {
            Object value = map.get(ExcelConfig.Convert.DEFAULT_KEY);
            //解析器实现，读取数据
            ...
            if (convName == null) {
                    ...
                    if (Type.EXPORT == type) {
                        ...
                        if (value instanceof String) {
                            ...
                            if (date == null) {
                                ...
                                errMsg.append(value.toString()).append("]").append("不能转换成日期,正确的格式应该是:[").append(
                                        pattern + "]");
                            ...
                            throw new ExcelException(getErrorMsg(fieldValue, "数据格式错误,[ " + name + " ]的类型是:"
                                    + value.getClass() + ",无法转换成日期", rowNum));
                        }
                    }
                } else if (format != null) {
                ...
                value = conv.convert(bean, map, fieldValue, type, rowNum);
                return value;
            }
        }
